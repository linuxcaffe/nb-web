...
31. **`effective_help`'s auto type-derived entry (`_resolve_help_list`, `app.py`, 2026-08-08) matches a note's literal `type:` value against a `.lib/help-type-<type>.md` filename — plural-sensitive, with no normalization.** `type: reports` looks for `help-type-reports.md` (plural); the project's own existing file is `help-type-report.md` (singular) — so `reports`-type notes get *no* automatic entry from this mechanism today, silently (a missing file is designed to contribute nothing, same as any other no-match case, not an error). `type: project` has no such mismatch (`help-type-project.md` matches exactly) and does auto-resolve. Before adding a new `.lib/help-type-<X>.md` file expecting every `type: X` note to pick it up automatically, check the file name against the note type's actual literal value (`_FM_TYPES` in `app.py`), not a display label or a guessed plural/singular form. `help_add:` (new cascading key, same accumulate-across-global→notebook→folder shape as `check_add:`/`cfg_attr_add:`, via `_collect_help_add`/`_collect_cascading_tokens`) and `help:` (existing nearest-wins) both still work independently of whether the auto entry matched — final order is auto-type → `help_add:` union → `help:`, deduplicated.
