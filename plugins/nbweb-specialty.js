...
(() => {
    ...
    // "Help Everywhere" — the ? button now lives in the always-visible preview toolbar
    // (#nb-help-btn), not the specialty header, so every note type gets one instead of
    // just project/report/dashboard/dotfile. See claude:nb-web_help-system_design.md.
    ...
    function _renderSpecialtyHeader(note) {
        ...
        // addBtn sits rightmost (its own margin-left:auto pushes it, and
        // extraActions after it, flush to the header's right edge) rather
        // than inline with pairLink/pillsHtml -- none of those are removed
        // or reordered, they just stay in their normal left-flowing spot.
    ...
    async function _renderDotfileNote(note) {
        ...
        return `<div class="nb-specialty-header" data-selector="${_esc(note.selector || '')}">
            ...
            <span class="nb-specialty-label">${_esc(label)}</span>
            ${dashLink}
            ${scopePill}${parentPill}${keysPill}${_addOrgBtnHtml(note)}
    ...
    async function _renderDashboardNote(note) {
        ...
        return `<div class="nb-specialty-header" data-selector="${_esc(note.selector || '')}">
            ...
            <span class="nb-specialty-right">
                ...
                ${syncPill}${accessBadge}${_addOrgBtnHtml(note)}
            </span>
        </div>` + NbMain.renderMarkdown(note.body || '', note.selector);
    }
...
<div class="nb-invoice-panel">
  ...
  <div class="nb-invoice-btns">
    ...
    NbWeb.registerModule('specialty', {
        ...
        // rendering depends on. `noRender: true` on a register() config is the general form of
        // the same thing -- added when nbweb-cine registered `storyline` purely to get it into
        // the nav popup (dashboard/project headers can jump to a production's storyline note,
        // and vice versa); without it, this previewRenderer started competing with cine's own
        // storyline-story/storyline-note renderers for the same note and silently won,
        // replacing cine's real board/story UI with a bare FM-field dump.
        previewRendererDetect: note => !!_cfg[note.type] && note.type !== 'item' && !_cfg[note.type]?.noRender,
        ...
        previewTypes: Object.keys(_cfg).filter(t => t !== 'item' && !_cfg[t]?.noRender),
    });
